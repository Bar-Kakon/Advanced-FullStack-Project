 ...
 *
 * Each earlier static screen was its own document with its own `<title>`, and this application has
 * one, so without this every screen would still be announcing "Create account". The titles carry
 * both languages, exactly as the earlier screens' did — a tab title is read at a glance, and a person
