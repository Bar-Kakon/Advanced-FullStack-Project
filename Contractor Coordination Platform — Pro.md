...
| Public profile | 2 | In progress | How another contractor sees you. **A feature, not a standalone route** — this restores the original registry note and supersedes the "a screen with its own route" wording it had drifted into; see [§11](#11-decision-log). Static HTML/CSS on `feature/browse-contractors-screen` (`b07fd30`), built **inside `screens/browse-contractors.html`**, with no file of its own. Opens as the **third column of `.browse__body`**, so the card grid narrows from 3-up to 2-up beside it and is never covered. Selection is CSS-only (`:target`), one panel per card, nine panels. Carries every My-profile section (rating · flexibility · About · Completed work · details · `schedulingPrefs` · ratings) plus the others'-view actions: Connect/Message, Rate (only when shared completed work makes the viewer eligible), Report, Mute, **Block**. Scope carries `schedulingPrefs` per the 2026-08-18 decision |
