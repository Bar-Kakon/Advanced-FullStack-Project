...
| My profile | 2 | In progress | Static HTML/CSS prototype on `feature/my-profile-screen` (`screens/my-profile.html` + `screens/my-profile.css`, `d7967a3`): read-only trust panel (rating + flexibility, including the cold-start state), identity editing (avatar stub, name, phone, bio, **multi-select** specialties, city + region + travel radius), scheduling preferences, and the received-ratings list. **Zero new JavaScript** — reuses the tracked `lang.js` and `validation.js`. Four product decisions settled with the owner; see [§11](#11-decision-log). Awaiting review |
