...
| Browse contractors | 2 | In progress | Static HTML/CSS prototype done (`screens/browse-contractors.html`): reusable navbar, filter rail (specialty/region/rating/flexibility), rich cards with rating + flexibility gauge + connection states. Real filtering/search/pagination wired in React (Stage 2). **2026-08-18:** gained a `Nationwide (all Israel)` region option; the Hebrew no-filter label was renamed `כל האזורים` because it already read `כל הארץ`. The Stage-2 query must match `region === selected OR region === 'nationwide'` |
...
- Fix: gave the chip `id="stat-updated"` and filled it at boot from `document.lastModified`, formatted from **local** date parts.
- Notes: correct over `file://`, where `document.lastModified` is the file's own mtime. **Served over HTTP without a `Last-Modified` header it returns the current time**, so a hosted copy would always read "today" — revisit if this document is ever served. The static date left in the markup is a deliberate fallback for scripts-disabled, and is the only part that can still age. Local date parts rather than `toISOString()`, because at +03:00 a save between midnight and 03:00 would render in UTC as the previous day.
