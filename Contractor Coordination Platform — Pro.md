...
| D11 | **Subscription data model & quota enforcement** | Nothing about plans exists in the schema today — `docs/database-design.html` has no plan field and no subscription collection. Undecided: whether the plan is a `users.plan` enum (simple, but keeps no billing history) **or** a separate `subscriptions` collection (billing periods, cancellation dates, upgrade/downgrade history — unavoidable once a real payment provider is involved); **where quotas are enforced**, which is the same write-side **authorization** concern §8 already flags and must be centralized rather than scattered across controllers; how the weekly cascade-simulation counter is stored and reset (rolling window vs. fixed week boundary; per-user vs. per-project); what "read-only project" concretely means on downgrade; and whether billing currency derives from a `users.country` rather than `users.language` — §3.4 separates language from locale, and the prototype couples them only because a static page has no other signal. | Stage 2 (screen) · later (billing) | Open |
...
- **Demo seed data is a deliverable risk.** The Live Demo is graded, and a multi-party coordination
  ...
  the night before.
- **Compiled JavaScript is tracked inconsistently across branches.** `screens/*.js` is gitignored on
  `feature/login-screen` and `feature/register-screen`, but **not** on `develop`/`main` — which track
  `screens/register.js`, and now `screens/lang.js`, as ordinary files. The same artifact is therefore a
  build output on one branch and source on another. The cause is structural, not careless: the `.ts`
  sources and `tsconfig.json` live on feature branches that were never merged, so a `develop`-based branch
  has no way to build them. **Do not "fix" this with a `.gitignore` edit alone** — an ignore rule has no
  effect on already-tracked paths, so it would need `git rm --cached`, and that would leave every static
  screen that loads compiled JS broken from a fresh clone. It resolves properly when the screen branches
  integrate and the `.ts` sources plus `tsconfig.json` finally live on one branch.
